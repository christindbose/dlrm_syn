...
class DLRM_Net(nn.Module):
    def create_mlp(self, ln, sigmoid_layer):
        ...
        for i in range(0, ln.size - 1):
            ...
            # approach 1
            #LL.weight.data = torch.from_numpy(W).requires_grad_()
        ...
        for k, sparse_index_group_batch in enumerate(lS_i):
            ...
            if self.quantize_emb:
                ...
                if k == 0: 
                    ...
                    print("row 3 base address of table ", k, hex(E.weight[3][0].data_ptr()), "shape ", E.weight.size(), "in device", E.weight.device)
                    print("row 0 base address of table ", k, hex(E.weight[0][0].data_ptr()), "shape ", E.weight.size(), "in device", E.weight.device)
                """
                ly.append(V)
    ...
    def parallel_forward(self, dense_x, lS_o, lS_i):
        ...
        ### Embedding lookup for UVM
        def _backward_tensor_hook_d0(grad):
            nvtx.range_push("EMB scatter backward0")
            grad.to_device("cuda:0")
            for device_id in device_ids:
                torch.cuda.synchronize(device_id)
            nvtx.range_pop()
        def _backward_tensor_hook_d1(grad):
            nvtx.range_push("EMB scatter backward1")
            grad.to_device("cuda:1")
            for device_id in device_ids:
                torch.cuda.synchronize(device_id)
            nvtx.range_pop()
        def _backward_tensor_hook_d2(grad):
            nvtx.range_push("EMB scatter backward2")
            grad.to_device("cuda:2")
            for device_id in device_ids:
                torch.cuda.synchronize(device_id)
            nvtx.range_pop()
        def _backward_tensor_hook_d3(grad):
            nvtx.range_push("EMB scatter backward3")
            grad.to_device("cuda:3")
            for device_id in device_ids:
                torch.cuda.synchronize(device_id)
            nvtx.range_pop()
        def _backward_tensor_hook_d4(grad):
            nvtx.range_push("EMB scatter backward0")
            grad.to_device("cuda:4")
            for device_id in device_ids:
                torch.cuda.synchronize(device_id)
            nvtx.range_pop()
        def _backward_tensor_hook_d5(grad):
            nvtx.range_push("EMB scatter backward1")
            grad.to_device("cuda:5")
            for device_id in device_ids:
                torch.cuda.synchronize(device_id)
            nvtx.range_pop()
        def _backward_tensor_hook_d6(grad):
            nvtx.range_push("EMB scatter backward2")
            grad.to_device("cuda:6")
            for device_id in device_ids:
                torch.cuda.synchronize(device_id)
            nvtx.range_pop()
        def _backward_tensor_hook_d7(grad):
            nvtx.range_push("EMB scatter backward3")
            grad.to_device("cuda:7")
            for device_id in device_ids:
                torch.cuda.synchronize(device_id)
            nvtx.range_pop()
        ...
        for device_id in device_ids:
            ...
            nvtx.range_push("Device " + str(device_id) + " build offset")
            for f, offsets in enumerate(lS_o):
                #print(device_id, f, offsets)
                if device_id == ndevices - 1:
                    one_lS_o.append(offsets[slice_size*(ndevices - 1) : ])#.to_device("cuda:"+str(device_id)))
                else:
                    one_lS_o.append(offsets[slice_size*device_id : slice_size*(device_id+1)])#.to_device("cuda:"+str(device_id)))
                #print(one_lS_o[-1])
            nvtx.range_pop()
            #print("len(ls_o):", len(one_lS_o), one_lS_o[0])

            nvtx.range_push("Device " + str(device_id) + " build input")
            for k, inputs in enumerate(lS_i):
                if device_id == ndevices - 1:
                    one_lS_i.append(inputs[one_lS_o[k][0] : ].to_device("cuda:"+str(device_id)))
                else:
                    one_lS_i.append(inputs[one_lS_o[k][0] : one_lS_o[k][-1]+1].to_device("cuda:"+str(device_id)))
            nvtx.range_pop()

            nvtx.range_push("Device " + str(device_id) + " copy offset")
            new_lS_o = []
            if device_id:
                for part_lS_o in one_lS_o:
                    #new_lS_o.append(part_lS_o.clone().sub_(part_lS_o[0]))
                    new_lS_o.append(part_lS_o.to_device("cuda:"+str(device_id)).clone().sub_(part_lS_o[0]))
            else:
                for part_lS_o in one_lS_o:
                    #new_lS_o.append(part_lS_o.clone())
                    new_lS_o.append(part_lS_o.to_device("cuda:"+str(device_id)).clone())
            nvtx.range_pop()
            ...
            for k, sy in enumerate(y):
                if k % ndevices == 0:
                    sy.register_hook(_backward_tensor_hook_d0)
                elif k % ndevices == 1:
                    sy.register_hook(_backward_tensor_hook_d1)
                elif k % ndevices == 2:
                    sy.register_hook(_backward_tensor_hook_d2)
                elif k % ndevices == 3:
                    sy.register_hook(_backward_tensor_hook_d3)
                elif k % ndevices == 4:
                    sy.register_hook(_backward_tensor_hook_d4)
                elif k % ndevices == 5:
                    sy.register_hook(_backward_tensor_hook_d5)
                elif k % ndevices == 6:
                    sy.register_hook(_backward_tensor_hook_d6)
                elif k % ndevices == 7:
                    sy.register_hook(_backward_tensor_hook_d7)
            #print("len(y): ", len(y))
        ...
        #ly = self.apply_emb(lS_o, lS_i, self.emb_l, self.v_W_l)
        ## debug prints
        ##print("len(ls_o):", len(lS_o), lS_o[0])
        ##print("len(ls_i):", len(lS_i), lS_i[0])
#
...
#
        ##for i, _ in enumerate(self.emb_l):
        ##    for t in ly[i]:
        ##        t.register_hook(_backward_tensor_hook)
        #
        #nvtx.range_push("EMB scatter forward")
        #t_list = []
        #for k, _ in enumerate(self.emb_l):
        #    d = torch.device("cuda:" + str(k % ndevices))
        #    #print("ly[k].shape:", ly[k].shape)
        #    y = scatter(ly[k], device_ids, dim=0)
        #    #print("len(y): ", len(y))
        #    #print("y[0].shape: ", y[0].shape)
        #    #print(y)
        #    #y = []
        #    #slice_size = (ly[k].shape[0]+ndevices-1) // ndevices
        #    #for device_id in device_ids:
        #    #    if device_id != ndevices - 1:
        #    #        y.append(ly[k][slice_size*device_id:slice_size*(device_id+1)].to_device("cuda:"+str(device_id)))
        #    #    else:
        #    #        y.append(ly[k][slice_size*device_id:].to_device("cuda:"+str(device_id)))
        #    
        #    t_list.append(y)
        ##for device in device_ids:
        ##    torch.cuda.synchronize(device)
        #nvtx.range_pop()
#
        ##for k, _ in enumerate(t_list):
        ##    for i, t in enumerate(t_list[k]):
        ##        if k % ndevices == 0:
        ##            t.register_hook(_backward_tensor_hook_d0)
        ##        elif k % ndevices == 1:
        ##            t.register_hook(_backward_tensor_hook_d1)
        ##        elif k % ndevices == 2:
        ##            t.register_hook(_backward_tensor_hook_d2)
        ##        elif k % ndevices == 3:
        ##            t.register_hook(_backward_tensor_hook_d3)
        ##        elif k % ndevices == 4:
        ##            sy.register_hook(_backward_tensor_hook_d4)
        ##        elif k % ndevices == 5:
        ##            sy.register_hook(_backward_tensor_hook_d5)
